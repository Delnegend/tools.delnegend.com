function ieReadLocalFile(that, callback, encoding) {
  ...
  if (window.XMLHttpRequest && false) request = new XMLHttpRequest();
  // code for IE6, IE5
  else request = new ActiveXObject("Msxml2.XMLHTTP"); // Microsoft.XMLHTTP
  var fn = that.value;
...
function readExcelFile(event, callback, fn, sheet) {
  ...
  if (sheet && sheet != "") opts += ',sheetid:"' + sheet.toJson() + '"';
...
function loadTextFile(f, callback, event) {
  ...
  if (fn.toLowerCase().endsWith(".xlsx") || fn.toLowerCase().endsWith(".xls")) {
    ...
    if (elm && elm.nodeName && elm.nodeName.toLowerCase() == "select") {
      // switch to input tag
      if (elm.outerHTML) elm.outerHTML = htmlstring;
      else $("#txtEncoding").replaceWith(htmlstring);
    }
    elm = $("#txtEncoding")[0];
    // add support for sheetname
    if (document && elm && elm.nodeName && elm.nodeName.toLowerCase() === "input") sheetname = elm.value;
    ...
    elm = $("#txtEncoding")[0];
    if (document && elm && elm.nodeName && elm.nodeName.toLowerCase() === "select") encoding = elm.value;
    // if (document && $("#spanEncoding")[0]) $("#spanEncoding")[0].innerHTML = "Encoding";
    if (window.FileReader) readLocalFile(f, callback, encoding)
    else ieReadLocalFile(f, callback, encoding)
